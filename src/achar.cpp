...
namespace AnsiGL
{
  ...
  //! Convert Color Codes Global Stub Method. Calls overloaded version.
  
  astring ConvertColorCodes( const ustring &str, const tColorCode *colorCodes )
  {
    ...
    for ( auto CurChar = s_begin; CurChar != s_end; ++CurChar )
      {
        ...
        // Check for ASCII control characters, etc.  Replace them with
        // a ' ' (space) glyph...but not ANSI escape codes
        ...
        if ( CheckChar == CurColorCodes[ColorCode_Escape] )
          {
            bool BreakLoop = true;  // Default is true...the list
                                    // start character sets false if
                                    // we need more than just one pass
            ...
            while ( CurChar != s_end )
              {
                ...
                if ( CheckChar == CurColorCodes[ColorCode_Escape] )
                          ...
                          || CheckChar == CurColorCodes[ColorCode_Grayscale] )
                  {
                    ...
                    // If the first non-digit is F or B, mark this
                    // color as such, and if it is neither, assume it
                    // to be F
            ...
            // Now that we've got that color code, just continue and
            // press on like normal
            continue;
          }
                ...
                for ( ++CurChar; CurChar != s_end; ++CurChar )
                  {
                    ...
                    else if ( CheckChar == ANSI_CODE_END || CheckChar == ']' ) // The
                                                                               // closing
                                                                               // bracket
                                                                               // is
                                                                               // optional,
